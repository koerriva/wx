...
void main()
{
    ...
    vec3 color_wo_sun = texture(tint2, vec2((sun_norm.y + 1.0) / 2.0,max(0.0,pos_norm.y))).rgb;
    vec3 color_w_sun = texture(tint, vec2((sun_norm.y + 1.0) / 2.0,max(0.0,pos_norm.y))).rgb;
...
//    float v = -uv.t;
    ...
    vec3 cloud_color = vec3(min(weather*3.0/2.0,1.0))*(sun_norm.y > 0 ? 0.98 : 0.98+sun_norm.y*1.8);
    ...
    float transparency = mix(texture(clouds2,vec2(u+time*0.05,v)).r,texture(clouds1,vec2(u+time*0.2,v+time*0.01)).r,(weather-0.5)*2.0);
    ...
    if(radius < 0.1){//We are in the area of the sky which is covered by the sun
